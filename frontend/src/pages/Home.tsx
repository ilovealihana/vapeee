...
<div class="copied-home-shell font-body-md text-body-md min-h-screen pb-24 selection:bg-primary/30">
  <main class="pt-2 px-margin-page space-y-stack-lg relative overflow-hidden">
    ...
    <section class="grid grid-cols-2 gap-stack-md relative z-10">
      <div class="bg-surface-container border border-outline-variant/30 rounded-xl p-4 flex flex-col gap-3 active-scale cursor-pointer transition-colors hover:bg-surface-container-high group text-left" role="button" tabindex="0" data-home-action="catalog">
      ...
      <div class="bg-surface-container border border-outline-variant/30 rounded-xl p-4 flex flex-col gap-3 active-scale cursor-pointer transition-colors hover:bg-surface-container-high text-left" role="button" tabindex="0" data-home-action="catalog">
        <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
    ...
    <section class="space-y-stack-md relative z-10">
      <div class="flex justify-between items-end">
        ...
        <button class="text-label-sm font-label-sm text-primary" type="button" data-home-action="catalog">${t('home.allCategories')}</button>
      </div>

      <div class="grid grid-cols-3 gap-stack-md">
        <div class="bg-surface-container-low border border-outline-variant/20 rounded-xl p-4 flex flex-col items-center gap-3 active-scale cursor-pointer transition-all hover:border-primary/40" role="button" tabindex="0" data-home-action="catalog">
    ...
    <section class="relative z-10">
      <div class="bg-primary/10 border border-primary/20 rounded-xl p-5 flex items-center justify-between overflow-hidden relative group active-scale cursor-pointer w-full text-left" role="button" tabindex="0" data-home-action="catalog">
    ...
    <section class="space-y-stack-md relative z-10">
      ...
      <div class="grid grid-cols-2 gap-stack-md">
        <div class="bg-surface-container rounded-xl overflow-hidden active-scale cursor-pointer flex flex-col text-left" role="button" tabindex="0" data-home-action="catalog">
          ...
          <div class="p-3 space-y-1">
            ...
            <div class="flex justify-between items-center pt-2">
              ...
              <span class="bg-secondary-container p-1 rounded-lg">
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onHomeClick = (event: Event) => {
      ...
      if (target?.closest('[data-home-action="catalog"]')) navigate('/cities');
    };
    const onHomeKeydown = (event: Event) => {
      const keyboardEvent = event as KeyboardEvent;
      const target = keyboardEvent.target as HTMLElement | null;
      if (!target?.closest('[data-home-action="catalog"]')) return;
    ...
    document.querySelector('.copied-home-shell')?.addEventListener('click', onHomeClick);
    document.querySelector('.copied-home-shell')?.addEventListener('keydown', onHomeKeydown);
    elements.forEach((el) => {
    ...
    return () => {
      ...
      document.querySelector('.copied-home-shell')?.removeEventListener('click', onHomeClick);
      document.querySelector('.copied-home-shell')?.removeEventListener('keydown', onHomeKeydown);
      elements.forEach((el) => {
